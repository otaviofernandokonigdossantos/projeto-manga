<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="mangas.css">
    <title>Document</title>
</head>
<body>
    <a href="paginaprincipal.php"><button>voltar</button></a><br><br> 
    <style>
    body{
        background: url(../projeto/img/demon.jpg);
        min-height: 200vh; 
        background-repeat: no-repeat;
        background-size: cover;
        
    }
    button{
        background-color: rgb(255, 115, 0);
    }</style>
   
<body>
    <div class="container">
        <div class="taiju.jpg"></div>
        <div class="madara.jpg"></div>
        <div class="dante.jpg"></div>
        <div class="Scorpion.jpg"></div>
       </div>
       <div id="div">
        <table  id = "tab" >
            <tr id="tr">
                <td id="a">
                <img src="img/leonidas.jpg" id="img">
                </td>
                <td id="b">
                <img src="img/bem10.jpg" id="img">
                </td>
                <td id="c">
                <img src="img/mutanterex.jpg" id="img">
                </td>
                <td id="d">
                <img src="img/aizen.jpg" id="img">
                </td>    
                <page  id = "e" >
                <img src="img/rain.jpg" id="img">
                </td>
                
            </tr>   
        </table>
        <table  id = "tab">
            <tr id="tr">
                <td id="f">
                <img src="img/toji.jpg" id="img">
                </td>
                <td id="g">
                <img src="img/obito.jpg" id="img">
                </td>
                <td id="h">
                <img src="img/taiju.jpg" id="img">
                </td>
                <td id="i">
                <img src="img/baki.jpg" id="img">
                </td>    
                <td id="j">
                <img src="img/ashura.jpg" id="img">
                </td>
                
            </tr>   
        </table>
        <table  id = "tab">
            <tr id="tr">
                <td id="k">
                <img src="img/ira.jpg" id="img">
                </td>
                <td id="l">
                <img src="img/slayer.jpg" id="img">
                </td>
                <td id="m">
                <img src="img/yugi.jpg" id="img">
                </td>
                <td id="n">
                <img src="img/Scorpion.jpg" id="img">
                </td>    
                <page  id = "from" >
                <img src="img/dante.jpg" id="img">
                </td>
                
            </tr>   
        </table>
        <table  id = "tab" >
            <tr id="tr">
                <td id="p">
                <img src="img/vergil.jpg" id="img">
                </td>
                <td id="q">
                <img src="img/madara.jpg" id="img">
                </td>
                <td id="r">
                <img src="img/broly.jpg" id="img">
                </td>
                <td id="s">
                <img src="img/ichigo.jpg" id="img">
                </td>    
                <td id="t">
                <img src="img/raiden.jpg" id="img">
                </td>
                
            </tr>   
        </table>
        <table  id = "tab" >
            <tr id="tr">
                <td id="u">
                <img src="img/wolverine.jpg" id="img">
                </td>
                <td id="v">
                <img src="img/sukuna.jpg" id="img">
                </td>
                <td id="w">
                <img src="img/nero.jpg" id="img">
                </td>
                <td id="x">
                <img src="img/barbabranca.jpg" id="img">
                </td>    
                <td id="y">
                <img src="img/batman.jpg" id="img">
                </td>
                
            </tr>   
        </table>
    </div>
</body>
